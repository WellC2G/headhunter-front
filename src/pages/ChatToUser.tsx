...
const ChatToUser: React.FC = () => {
    ...
    useEffect(() => {
        const fethData = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:3000/chat/userId/${targetId}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    }
                });
    ...
    const isMyMessage = (message: Message): boolean => {
        ...
        return member?.companyId?.toString() === localStorage.getItem("companyId");
    };
    ...
    const handleSendMessage = async (e: any) => {
        ...
        try {
            ...
            const token = localStorage.getItem("token");
            const data = {content: newMessage, targetId: targetId};
            await axios.post(`http://localhost:3000/chat/send/company`, data , {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
